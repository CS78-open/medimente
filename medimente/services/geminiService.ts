...
// Rimossa l'importazione di API_KEY da '../config' poiché useremo process.env.API_KEY direttamente

// Rimossa l'istanza globale di GoogleGenAI.
// const ai = new GoogleGenAI({ apiKey: API_KEY });

// Rimossa la funzione ensureApiKeyIsSet e le sue chiamate.
  ...
  properties: {
    ...
    structuredSummary: {
      ...
      items: {
        type: Type.OBJECT,
        properties: {
          farmaco: { type: Type.STRING, description: 'Name of the medication.' },
          dosaggio: { type: Type.STRING, description: 'Dosage of the medication.' },
          oraImportante: { type: Type.STRING, description: 'Important time for medication (e.g., "mattina", "alle 8", "dopo cena").' },
          motivo: { type: Type.STRING, description: 'Reason for taking the medication.' },
        },
 ...
 */
export async function extractTextFromFile(mimeType: string, data: string): Promise<string> {
  try {
    const filePart = {
      inlineData: {
        mimeType,
    ...
    const response = await currentAi.models.generateContent({
      model: 'gemini-2.5-flash', // This model is multi-modal
 ...
 */
export async function generateMedicationReminder(medicalText: string): Promise<LLMOutput> {
  try {
    // Crea una nuova istanza di GoogleGenAI per ogni chiamata API
    const currentAi = new GoogleGenAI({ apiKey: process.env.API_KEY });
    const response = await currentAi.models.generateContent({
      model: 'gemini-2.5-flash', // Using gemini-2.5-flash for basic text tasks
      contents: [{ parts: [{ text: medicalText }] }],
      config: {
...
Assicurati che tutte le informazioni siano accurate e facili da capire. La storiella deve essere positiva e rassicurante.`,
    ...
    const llmOutput: LLMOutput = JSON.parse(jsonStr);
