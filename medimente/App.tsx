...
import { generateMedicationReminder, extractTextFromFile } from './services/geminiService';
import { LLMOutput, LLMState, MedicationEntry } from './types';
...
function App() {
  ...
  const [llmState, setLlmState] = useState<LLMState>(LLMState.IDLE);
  ...
  const handleGenerateReminder = useCallback(async () => {
    ...
    if (!medicalText.trim()) {
      setError("Per favore, inserisci le istruzioni mediche.");
  ...
  const handleAddSingleEventToCalendar = useCallback((entry: MedicationEntry) => {
  ...
  const handleSpeakStory = useCallback(() => {
    if (!('speechSynthesis' in window) || !llmOutput?.gentleStory) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex flex-col items-center p-4 sm:p-6 lg:p-8">
      <header className="w-full max-w-3xl text-center py-6">
        ...
        <p className="mt-4 text-lg text-blue-600">
          Il tuo assistente AI per istruzioni mediche chiare e promemoria personalizzati.
        </p>
      </header>

      <main className="flex-grow w-full max-w-3xl bg-white shadow-xl rounded-lg p-6 sm:p-8 lg:p-10 mb-8">
        <section className="mb-8">
          <label htmlFor="medical-instructions" className="block text-xl font-semibold text-gray-800 mb-3">
            1. Inserisci o carica le istruzioni mediche
          </label>
          <div className="mb-4">
            <input
              type="file"
        ...
        {llmOutput && llmState === LLMState.SUCCESS && (
          <section>
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">
              3. Il tuo promemoria personalizzato:
            </h2>

            <div className="mb-8 p-6 bg-blue-50 border-l-4 border-blue-400 rounded-lg shadow-inner">
              <h3 className="text-xl font-bold text-blue-700 mb-3">RIASSUNTO FACILE (Tabella):</h3>
              <div className="overflow-x-auto">
                <MarkdownRenderer className="text-gray-700">
                  {llmOutput.summaryTable}
                </MarkdownRenderer>
              </div>
            </div>

            <div className="mb-8 p-6 bg-purple-50 border-l-4 border-purple-400 rounded-lg shadow-inner">
              <h3 className="text-xl font-bold text-purple-700 mb-4">AGGIUNGI AL CALENDARIO:</h3>
              <div className="flex flex-col space-y-3">
                {llmOutput.structuredSummary.map((entry, index) => (
                  <button
                    key={index}
                  ...
                  >
                    ...
                    <span>Aggiungi promemoria per <strong>{entry.farmaco}</strong></span>
                  </button>
                ))}
              </div>
            </div>

            <div className="p-6 bg-green-50 border-l-4 border-green-400 rounded-lg shadow-inner">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-xl font-bold text-green-700">STORIELLA GENTILE:</h3>
                <button
                ...
                >
                  {isSpeaking ? (
                    <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                      <path d="M5 5a1 1 0 00-1 1v8a1 1 0 001 1h8a1 1 0 001-1V6a1 1 0 00-1-1H5z"></path>
                    </svg>
                  ) : (
      ...
      <footer className="w-full max-w-3xl text-center py-4">
        <p className="text-gray-600 text-sm">
          L'app è progettata da <a href="https://openssn.marcopingitore.it" target="_blank" rel="noopener noreferrer" className="underline hover:text-blue-700 transition-colors">Marco Pingitore</a>
        </p>
      </footer>
    </div>
  );
